...
await fs.mkdir(path.join(rootA, '.omp', 'rules'), { recursive: true });
await fs.mkdir(path.join(rootA, '.omp', 'skills', 'web-native'), { recursive: true });
await fs.writeFile(path.join(rootA, '.omp', 'SYSTEM.md'), '# Project OMP system\nUse ChatGPT Web native capabilities.\n');
await fs.writeFile(path.join(rootA, '.omp', 'rules', 'web.md'), '# Project OMP rule\nKeep direct MCP edits in the selected workspace.\n');
await fs.writeFile(path.join(rootA, '.omp', 'skills', 'web-native', 'SKILL.md'), '---\nname: web-native\ndescription: Native Web workflow\n---\nUse the ChatGPT Web conversation as the active model.\n');
...
try {
  ...
  try {
    ...
    const requiredTools = ['workspace', 'omp', 'task', 'agents', 'pi', 'codex', 'read', 'write', 'edit', 'apply_patch', 'bash', 'tree', 'search', 'show_changes'];
    for (const expected of requiredTools) if (!tools.includes(expected)) throw new Error(`missing ${expected}`);
    const ompTool = listedTools.find(tool => tool.name === 'omp');
    if (!ompTool?.description?.includes('OMP-native subset') || !ompTool?.description?.includes('does not launch another model')) throw new Error('OMP Web capability description missing');
    ...
    const ompStatus = await call(a.client, 'omp', { action: 'status' });
    if (ompStatus.runtime !== 'chatgpt-web-native' || ompStatus.external_omp_process !== false || ompStatus.native_tool_scope !== 'omp-native-only' || ompStatus.native_tools.includes('task') || ompStatus.native_tools.includes('pi') || ompStatus.native_tools.includes('codex')) throw new Error(`OMP unexpectedly launched a model runtime or advertised compatibility tools: ${JSON.stringify(ompStatus)}`);
    const ompContext = await call(a.client, 'omp', { action: 'context' });
    if (ompContext.runtime !== 'chatgpt-web-native' || ompContext.model_execution !== 'chatgpt-web' || ompContext.native_tool_scope !== 'omp-native-only' || !ompContext.native_tools.includes('read') || !ompContext.native_tools.includes('load_skill') || !ompContext.agents.some(item => item.name === 'scout') || ompContext.native_tools.includes('task') || ompContext.native_tools.includes('pi') || ompContext.native_tools.includes('codex') || !ompContext.skills.some(item => item.name === 'web-native') || !ompContext.files.some(item => item.kind === 'system' && item.path === '$WORKSPACE/.omp/SYSTEM.md') || !ompContext.files.some(item => item.kind === 'rule' && item.path === '$WORKSPACE/.omp/rules/web.md')) throw new Error(`OMP Web context failed: ${JSON.stringify(ompContext)}`);
    const ompSkill = await call(a.client, 'omp', { action: 'skill', skill_name: 'web-native', source: 'workspace' });
    if (ompSkill.skill?.name !== 'web-native' || !ompSkill.text.includes('active model')) throw new Error(`OMP Web skill loading failed: ${JSON.stringify(ompSkill)}`);
    const ompAgent = await call(a.client, 'omp', { action: 'agent', agent_name: 'scout' });
    if (ompAgent.agent?.name !== 'scout' || !ompAgent.agent?.system_prompt?.includes('read-only')) throw new Error(`OMP Web agent loading failed: ${JSON.stringify(ompAgent)}`);
    const agents = await call(a.client, 'agents', { action: 'list' });
    if (!agents.agents.some(item => item.name === 'scout') || !agents.backends.includes('omp')) throw new Error(`agent inventory failed: ${JSON.stringify(agents)}`);
    const taskStatus = await call(a.client, 'task', { action: 'status' });
    if (taskStatus.max_concurrency !== 1 || taskStatus.state !== 'idle') throw new Error(`generic task status failed: ${JSON.stringify(taskStatus)}`);
    ...
    const piTool = listedTools.find(tool => tool.name === 'pi');
    if (!piTool?.description?.includes('No h5i')) throw new Error('pi tool is not the bounded on-demand runtime');
    ...
    console.log(JSON.stringify({ tools: requiredTools, chat_a: currentA.workspace.root, chat_b: currentB.workspace.root, same_endpoint: true, direct_edit: true, direct_bash: true, repository_discovery: true, reconnect_safe: true, persistent_agents: false, on_demand_agents: true }, null, 2));
  } finally {
