...
function usage() {
  console.log(`Agent Loom easy launcher

Usage:
  npm install -g agent-loom
  agent-loom setup
  agent-loom start
  agent-loom start --root /path/to/repo
  agent-loom settings
  agent-loom doctor
  agent-loom connection-test --root /path/to/repo
  agent-loom inspect --root /path/to/repo [--json]
  agent-loom review --root /path/to/repo [--staged] [--path src/file.ts] [--json]
  agent-loom execute-handoff --agent opencode --model provider/model
  agent-loom watch-handoff --agent opencode --model provider/model
  agent-loom loop-handoff --agent opencode --model provider/model --review-command "node ./reviewer.js --status {{status_file}} --diff {{diff_file}} --plan-file {{plan_file}}"
  agent-loom --root /path/to/repo
  agent-loom ngrok --hostname your-domain.ngrok-free.dev
  agent-loom tailscale --hostname your-device.your-tailnet.ts.net
  agent-loom stable --hostname agent-loom.example.com --tunnel-name agent-loom
  agent-loom pro-bundle --root /path/to/repo --copy
  agent-loom pro-apply --root /path/to/repo --file plan.md
  agent-loom install-cloudflared
  npm run connect -- --root /path/to/repo
  node scripts/agent-loom.mjs --root /path/to/repo --tunnel cloudflare
...
Options:
  ...
  --cloudflared <path>      cloudflared executable. Default: PATH, then ~/.agent-loom/bin.
  ...
  --no-profile              Do not load a saved ~/.agent-loom workspace profile.
  ...
  --install-cloudflared     Install/reinstall cloudflared into ~/.agent-loom/bin.
  ...
  --version, -v             Print the Agent Loom version.
  --help                    Show this message.

Execute handoff options:
  agent-loom execute-handoff --agent opencode --model provider/model
  agent-loom execute-handoff --agent pi --model provider/model
  agent-loom execute-handoff --agent custom --command "my-agent --task-file {{plan_file}}"
...
Watch handoff options:
  agent-loom watch-handoff --agent opencode --model provider/model
  agent-loom watch-handoff --agent pi --model provider/model
  agent-loom watch-handoff --agent custom --command "my-agent --task-file {{plan_file}}"
...
Loop handoff options:
  agent-loom loop-handoff --agent opencode --model provider/model --review-command "reviewer --status {{status_file}} --diff {{diff_file}} --plan-file {{plan_file}}"
  --review-command <template>
                             Local reviewer/orchestrator command. It should print AGENT_LOOM_REVIEW=PASS or AGENT_LOOM_REVIEW=FAIL.
  ...
  --allow-implicit-review-verdict
                             Infer PASS/FAIL from reviewer exit code and plan changes when no AGENT_LOOM_REVIEW line is printed.
...
Workspace settings:
  agent-loom settings
  agent-loom settings show
  agent-loom settings list
  agent-loom settings set --tunnel ngrok --hostname your-domain.ngrok-free.dev
  agent-loom settings set --project /path/to/another/repo
  agent-loom settings set --clear-projects
  agent-loom settings use
  agent-loom settings delete --yes

Preflight diagnostics:
  agent-loom doctor

Ngrok stable URL mode:
  agent-loom ngrok --root /path/to/repo --hostname your-domain.ngrok-free.dev

Tailscale Funnel mode:
  agent-loom tailscale --root /path/to/repo --hostname your-device.your-tailnet.ts.net

Planning-only handoff mode:
  agent-loom start --root /path/to/repo --mode handoff

Execute a local handoff after ChatGPT writes .ai-bridge/current-plan.md:
  agent-loom execute-handoff --agent opencode --model provider/model
  agent-loom execute-handoff --agent pi --model provider/model
  agent-loom execute-handoff --agent custom --command "node ./agent.js --task-file {{plan_file}}" --yes

Watch for new handoff plans and execute them locally:
  agent-loom watch-handoff --agent opencode --model provider/model --yes
  agent-loom watch-handoff --agent custom --command "node ./agent.js --task-file {{plan_file}}" --yes

Run a bounded local execute/review loop:
  agent-loom loop-handoff --agent opencode --model provider/model --review-command "node ./reviewer.js --status {{status_file}} --diff {{diff_file}} --plan-file {{plan_file}}" --max-iters 3 --yes

Stable URL mode after one-time Cloudflare tunnel setup:
  agent-loom stable --root /path/to/repo --hostname agent-loom.example.com --tunnel-name agent-loom
`);
}
...
function printSavedProfileHint(profile) {
  ...
  printBox('Saved setup found', [
    summary,
    'From this folder, future launches only need: agent-loom start',
    'Use agent-loom setup when you want to change the port, mode, tool mode, tunnel, hostname, or token.'
  ]);
}
...
function printWorkspaceInspection(result, json) {
  ...
  console.log([
    'Agent Loom Repository Analysis',
    '',
...
function writeOption(args, profile, mode) {
  return effectiveWriteMode(mode, optionValue(args, profile, 'write', ['AGENT_LOOM_WRITE_MODE'], mode === 'agent' ? 'workspace' : 'handoff'));
}
...
function optionalWriteOption(args, profile, mode) {
  const requested = optionValue(args, profile, 'write', ['AGENT_LOOM_WRITE_MODE'], '');
...
function resolveCodexCommand() {
  const explicit = String(process.env.AGENT_LOOM_CODEX_BIN ?? '').trim();
  if (explicit) {
...
function codexProHome() {
  const customHome = process.env.AGENT_LOOM_HOME;
  return customHome ? path.resolve(expandHome(customHome)) : path.join(os.homedir(), '.agent-loom');
}
...
function saveRuntimeConnection(root, details, options = {}) {
  ...
  const payload = {
    ...
    localStatusUrl: details.localStatusUrl ? details.localStatusUrl.replace(/agent_loom_token=[^&]+/, 'agent_loom_token=<redacted>') : '',
...
function hasToolCardsInput(args, profile = {}) {
  return args.toolCards !== undefined || profile.toolCards !== undefined || (process.env.AGENT_LOOM_TOOL_CARDS !== undefined && process.env.AGENT_LOOM_TOOL_CARDS !== '');
}

function toolCardsProfileEntry(args, profile = {}) {
  const hasInput = hasToolCardsInput(args, profile);
  return hasInput ? { toolCards: optionBool(args, profile, 'toolCards', ['AGENT_LOOM_TOOL_CARDS'], false) } : {};
}

function toolCardsCliArgs(args, profile = {}) {
  if (!hasToolCardsInput(args, profile)) return [];
  return ['--tool-cards', optionBool(args, profile, 'toolCards', ['AGENT_LOOM_TOOL_CARDS'], false) ? 'on' : 'off'];
}
...
function bashSessionOptions(args, profile = {}) {
  const bashSession = validateBashSession(optionValue(args, profile, 'bashSession', ['AGENT_LOOM_BASH_SESSION_ID'], ''));
  const requireBashSession = optionBool(args, profile, 'requireBashSession', ['AGENT_LOOM_REQUIRE_BASH_SESSION'], false);
...
function bashTranscriptOption(args, profile = {}) {
  const value = optionValue(args, profile, 'bashTranscript', ['AGENT_LOOM_BASH_TRANSCRIPT'], 'compact');
...
function codexSessionsOption(args, profile = {}) {
  const value = optionValue(args, profile, 'codexSessions', ['AGENT_LOOM_CODEX_SESSIONS'], 'off');
...
async function downloadFile(url, destination, asset) {
  const response = await fetch(url, {
    headers: { 'user-agent': 'agent-loom-launcher' }
  });
  if (!response.ok) {
...
async function installCloudflaredLocal() {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-loom-cloudflared-'));
  const url = cloudflaredReleaseUrl(asset);

  fs.mkdirSync(binDir, { recursive: true, mode: 0o700 });
  console.error(`[agent-loom] Installing cloudflared locally: ${installPath}`);
  console.error(`[agent-loom] Downloading verified Cloudflare release ${CLOUDFLARED_VERSION}: ${asset.file}`);
  ...
  try {
    ...
    console.error('[agent-loom] cloudflared installed successfully.');
    return installPath;
  } finally {
...
async function resolveCloudflared(args) {
  ...
  if (!args.installCloudflared && commandExists('cloudflared')) {
    try {
      ...
      console.error(`[agent-loom] cloudflared in PATH failed --version; trying local install. ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  if (!args.installCloudflared && executableFileExists(localPath)) {
    try {
      ...
      console.error(`[agent-loom] Existing ${localPath} failed --version; reinstalling. ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
function resolveTailscale(args) {
  ...
  throw new Error('tailscale was not found on PATH. Install Tailscale and enable Funnel, then run agent-loom tailscale --hostname your-device.your-tailnet.ts.net.');
}

function ngrokConfigPath(root, args, profile = {}) {
  const configPath = optionValue(args, profile, 'ngrokConfig', ['NGROK_CONFIG', 'AGENT_LOOM_NGROK_CONFIG'], '');
...
function runHelperScript(scriptName, args) {
  ...
  const result = spawnSync(process.execPath, [scriptPath, ...args], {
    cwd: projectRoot,
    env: { ...process.env, AGENT_LOOM_CALLER_CWD: process.cwd() },
    stdio: 'inherit'
  });
...
function portInUseHelp(host, port) {
  return [
    ...
    '',
    'Example:',
    '  repo A: agent-loom setup  -> port 8787 -> hostname A',
    '  repo B: agent-loom setup  -> port 8788 -> hostname B',
    '',
    'For quick tunnels you can also start the second repo with:',
    '  agent-loom start --port 8788',
    '',
...
function spawnLogged(name, command, args, options = {}) {
  ...
  child.agentLoomKillTree = Boolean(invocation.killTree);
  const logLines = [];
  ...
  child.agentLoomLogTail = () => logLines.join('\n');
...
function waitForTunnelStartup(child, label, timeoutMs = 1000) {
  return new Promise((resolve, reject) => {
    ...
    const outputTail = () => {
      const tail = typeof child.agentLoomLogTail === 'function' ? child.agentLoomLogTail() : '';
      return tail ? `\n\nRecent ${label} output:\n${tail}` : '';
    };
...
function writeQuickTunnelCredentials(tunnel) {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-loom-cloudflare-quick-'));
...
function killProcess(child) {
  ...
  if (child.agentLoomKillTree && child.pid) {
...
function endpointWithToken(endpoint, token) {
  ...
  url.searchParams.set('agent_loom_token', token);
  return url.toString();
}
...
function normalizeMode(args) {
  const mode = args.mode ?? process.env.AGENT_LOOM_MODE ?? 'agent';
...
function contextDirFromArgs(args) {
  return args.contextDir ?? process.env.AGENT_LOOM_CONTEXT_DIR ?? '.ai-bridge';
}
...
function handoffMaxReadBytes() {
  return numberOption(process.env.AGENT_LOOM_MAX_READ_BYTES, 180_000, 4_000, 2_000_000);
}
...
function redactForLog(value) {
  return String(value)
    ...
    .replace(/([?&](?:agent_loom_token|token|access_token|auth_token|api[_-]?key)=)[^&\s"'`<>]{8,}/gi, '$1[REDACTED_SECRET]')
...
function buildExecutorCommand(args, root, planPath, planText) {
  ...
  const model = String(args.model ?? process.env.AGENT_LOOM_AGENT_MODEL ?? '').trim();
  const replacements = {
    model,
    plan_file: planPath,
    plan_text: planText,
...
function runProcessCaptured(command, args, options) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (exitCode, signal) => {
      ...
      const err = trimBytes(`${stderr}${timedOut ? `\n[agent-loom] Command timed out after ${timeoutMs} ms.` : ''}`, maxOutputBytes);
      resolve({
        exitCode,
        signal,
        durationMs: Date.now() - started,
...
async function confirmLocalExecution(args, root, commandInfo) {
  ...
  printBox('Confirm local execution', [
    ...
    'This runs a local process in the workspace. Agent Loom will collect status, logs, and git diff into .ai-bridge.'
  ]);
...
function loadHandoffExecution(args) {
  const root = realDir(args.root ?? process.env.AGENT_LOOM_ROOT ?? process.cwd());
  ...
  const maxOutputBytes = numberOption(args.maxOutputBytes ?? process.env.AGENT_LOOM_MAX_OUTPUT_BYTES, 120_000, 4_000, 2_000_000);
...
function printHandoffDryRun(request, title = 'Agent Loom execute-handoff dry run') {
  printBox(title, [
...
async function runWatchHandoff(argv) {
  ...
  printBox('Agent Loom watch-handoff', [
    labelValue('Workspace', root),
  ...
  while (!stopped) {
    ...
    if (args.dryRun) {
      printHandoffDryRun(request, 'Agent Loom watch-handoff dry run');
...
function explicitReviewVerdict(text) {
  for (const rawLine of String(text || '').split(/\r?\n/)) {
    const line = rawLine.trim();
    const assignment = line.match(/^AGENT_LOOM_REVIEW\s*=\s*(PASS|FAIL)\b/i);
...
function printLoopDryRun(request, reviewCommand, testCommand, maxIters) {
  printBox('Agent Loom loop-handoff dry run', [
    labelValue('Workspace', request.root),
    labelValue('Plan', path.relative(request.root, request.planPath)),
    labelValue('Agent', request.commandInfo.agent),
    ...(request.commandInfo.model ? [labelValue('Model', request.commandInfo.model)] : []),
...
async function runLoopHandoff(argv) {
  ...
  printBox('Agent Loom loop-handoff', [
    labelValue('Workspace', root),
...
function printConnectorBlock(endpoint, token, options = {}) {
  ...
  console.log('');
  console.log(paint('bold', 'Agent Loom ready'));
  ...
  if (options.connectionTest) {
    ...
    console.log('  3. Watch this terminal for: [Agent Loom] POST /mcp received');
    console.log('');
    console.log('  No POST /mcp     ChatGPT or the tunnel did not reach Agent Loom.');
    console.log('  POST /mcp -> 401 The full Server URL, including agent_loom_token, was not used.');
    console.log('  POST /mcp -> 2xx The MCP connection reached Agent Loom successfully.');
    console.log('');
  }
  if (options.headless) {
    console.log(`AGENT_LOOM_READY ${serverUrl}`);
  } else {
...
function printModeHelp() {
  console.log('');
  console.log('Modes');
  console.log('  agent-loom start                 agent mode: read/write/edit/apply_patch/search/bash');
  console.log('  agent-loom start --no-bash       agent mode without ChatGPT-triggered shell commands');
  console.log('  agent-loom start --bash-session main --require-bash-session');
  console.log('  agent-loom start --mode handoff  planning-only .ai-bridge handoff');
  console.log('  agent-loom start --mode pro      export context for models without MCP tools');
  console.log('  agent-loom start --tool-mode minimal   expose only the tight coding loop');
  console.log('  agent-loom start --tool-mode full      expose every advanced compatibility tool');
  console.log('');
}
...
function printStableUrlHelp() {
  ...
  console.log('  agent-loom install-cloudflared');
  console.log('  ~/.agent-loom/bin/cloudflared tunnel login');
  console.log('  ~/.agent-loom/bin/cloudflared tunnel create agent-loom');
  console.log('  ~/.agent-loom/bin/cloudflared tunnel route dns agent-loom agent-loom.example.com');
  console.log('');
  console.log('Daily start:');
  console.log('  agent-loom stable --hostname agent-loom.example.com --tunnel-name agent-loom --token keep-this-stable-token');
  console.log('');
  console.log('Ngrok alternative with a reserved domain:');
  console.log('  ngrok config add-authtoken <your-ngrok-token>');
  console.log('  agent-loom ngrok --hostname your-domain.ngrok-free.dev --token keep-this-stable-token');
  console.log('');
  console.log('Tailscale Funnel alternative:');
  console.log('  tailscale funnel 8787');
  console.log('  agent-loom tailscale --hostname your-device.your-tailnet.ts.net --token keep-this-stable-token');
  console.log('');
}
...
async function runDoctor(argv) {
  ...
  const tunnel = optionValue(args, profile, 'tunnel', ['AGENT_LOOM_TUNNEL'], 'cloudflare');
  const host = optionValue(args, profile, 'host', ['AGENT_LOOM_HOST'], '127.0.0.1');
  const port = String(optionValue(args, profile, 'port', ['AGENT_LOOM_PORT'], '8787'));
  const mode = optionValue(args, profile, 'mode', ['AGENT_LOOM_MODE'], 'agent');
  const bash = optionValue(args, profile, 'bash', ['AGENT_LOOM_BASH_MODE'], 'safe');
  const rawWrite = optionValue(args, profile, 'write', ['AGENT_LOOM_WRITE_MODE'], mode === 'agent' ? 'workspace' : 'handoff');
  ...
  const toolMode = optionValue(args, profile, 'toolMode', ['AGENT_LOOM_TOOL_MODE'], 'standard');
  const stableHostname = args.hostname
    ?? args.url
    ?? process.env.AGENT_LOOM_PUBLIC_HOSTNAME
    ?? process.env.AGENT_LOOM_HOSTNAME
  ...
  if (tunnel === 'none') {
    ...
    record(cloudflaredPath ? 'ok' : 'warn', 'cloudflared', cloudflaredPath || 'missing now; agent-loom start can auto-install unless --no-install-cloudflared is used');
  } else if (tunnel === 'cloudflare-named') {
    record(stableHostname ? 'ok' : 'fail', 'Hostname', stableHostname || 'required for Cloudflare stable mode');
    record(cloudflaredPath ? 'ok' : 'warn', 'cloudflared', cloudflaredPath || 'missing now; run agent-loom install-cloudflared or pass --cloudflared');
    record(
      optionValue(args, profile, 'tunnelName', ['CLOUDFLARE_TUNNEL_NAME', 'AGENT_LOOM_TUNNEL_NAME'], '') ||
        optionValue(args, profile, 'cloudflareTokenFile', ['CLOUDFLARE_TUNNEL_TOKEN_FILE', 'AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN_FILE'], '') ||
        optionValue(args, profile, 'cloudflareConfig', ['CLOUDFLARE_TUNNEL_CONFIG', 'AGENT_LOOM_CLOUDFLARE_CONFIG'], '') ||
        optionValue(args, profile, 'cloudflareToken', ['CLOUDFLARE_TUNNEL_TOKEN', 'AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN'], '')
        ? 'ok'
        : 'fail',
      'Cloudflare setup',
...
async function collectTunnelPreference(rl, defaults, profile, options = {}) {
  ...
  if (tunnel === 'ngrok') {
    hostname = await ask(
      ...
      optionValue(defaults, profile, 'hostname', ['AGENT_LOOM_PUBLIC_HOSTNAME', 'AGENT_LOOM_HOSTNAME', 'NGROK_DOMAIN'], '')
    );
    ...
    ngrokConfig = optionValue(defaults, profile, 'ngrokConfig', ['NGROK_CONFIG', 'AGENT_LOOM_NGROK_CONFIG'], '');
  } else if (tunnel === 'cloudflare-named') {
    hostname = await ask(
      rl,
      'Stable Cloudflare hostname, without /mcp',
      optionValue(defaults, profile, 'hostname', ['AGENT_LOOM_PUBLIC_HOSTNAME', 'AGENT_LOOM_HOSTNAME'], '')
    );
    if (!hostname) throw new Error('Stable public URL setup needs a real hostname, for example agent-loom.yourdomain.com.');
    hostname = normalizePublicHostname(hostname);
    tunnelName = await ask(rl, 'Cloudflare tunnel name', optionValue(defaults, profile, 'tunnelName', ['AGENT_LOOM_TUNNEL_NAME', 'CLOUDFLARE_TUNNEL_NAME'], 'agent-loom'));
    cloudflareConfig = optionValue(defaults, profile, 'cloudflareConfig', ['AGENT_LOOM_CLOUDFLARE_CONFIG', 'CLOUDFLARE_TUNNEL_CONFIG'], '');
    cloudflareTokenFile = optionValue(defaults, profile, 'cloudflareTokenFile', ['AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CLOUDFLARE_TUNNEL_TOKEN_FILE'], '');
  } else if (tunnel === 'tailscale') {
    hostname = await ask(
      rl,
      'Tailscale Funnel hostname, without /mcp',
      optionValue(defaults, profile, 'hostname', ['AGENT_LOOM_PUBLIC_HOSTNAME', 'AGENT_LOOM_HOSTNAME', 'TAILSCALE_FUNNEL_HOSTNAME'], '')
    );
...
function profileFromPreference(root, args, profile, preference) {
  const mode = optionValue(args, profile, 'mode', ['AGENT_LOOM_MODE'], 'agent');
  const port = String(optionValue(args, profile, 'port', ['AGENT_LOOM_PORT'], '8787'));
  const bash = optionValue(args, profile, 'bash', ['AGENT_LOOM_BASH_MODE'], '');
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  const codexDir = optionValue(args, profile, 'codexDir', ['AGENT_LOOM_CODEX_DIR'], '');
  const { bashSession, requireBashSession } = bashSessionOptions(args, profile);
  const write = optionalWriteOption(args, profile, mode);
  const toolMode = optionValue(args, profile, 'toolMode', ['AGENT_LOOM_TOOL_MODE'], '');
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['AGENT_LOOM_WIDGET_DOMAIN'], '');
  const existingToken = optionValue(args, profile, 'token', ['AGENT_LOOM_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], '');
...
async function maybeConfigureFirstRun(root, args, profile) {
  ...
  if (reusableProfiles.length) {
    ...
    printBox('Saved setups', [
      'No saved settings exist for this workspace, but Agent Loom found saved setups from other workspaces.',
    ...
    try {
      ...
      if (Number.isInteger(selectedIndex) && selectedIndex >= 1 && selectedIndex <= shown.length) {
        ...
        const payload = reusableProfilePayload(selected, {
          port: String(optionValue(args, selected, 'port', ['AGENT_LOOM_PORT'], selected.port ?? '8787')),
          mode: optionValue(args, selected, 'mode', ['AGENT_LOOM_MODE'], selected.mode ?? 'agent')
        });
  ...
  printBox('First run setup', [
    ...
    'Choose once now. Agent Loom will reuse this choice on future agent-loom start runs until you change or delete it with agent-loom settings.'
  ]);
...
function commandPreview(args) {
  return shellCommandPreview(['agent-loom', ...args]);
}

async function runSetupWizard(argv) {
  if (!process.stdin.isTTY) {
    throw new Error('agent-loom setup needs an interactive terminal. Use agent-loom start --root /path/to/repo for non-interactive scripts.');
  }
  const defaults = parseArgs(argv);
  const defaultRoot = path.resolve(expandHome(defaults.root ?? process.env.AGENT_LOOM_ROOT ?? process.cwd()));

  printBox('Agent Loom setup', [
    'This wizard prepares a ChatGPT connector for the folder you choose.',
    'Press Enter to accept defaults. Stable tunnel choices are saved per workspace under ~/.agent-loom.'
  ]);
  ...
  try {
    ...
    const savedTunnel = optionValue(defaults, profile, 'tunnel', ['AGENT_LOOM_TUNNEL'], 'cloudflare');
  const defaultTunnel = savedTunnel === 'cloudflare-named'
      ...
      : savedTunnel === 'ngrok'
        ...
        : savedTunnel === 'tailscale'
          ...
          : savedTunnel === 'none'
            ? 'local'
            : 'quick';
    const defaultPort = String(optionValue(defaults, profile, 'port', ['AGENT_LOOM_PORT'], '8787'));
    const defaultMode = normalizeSetupChoice(optionValue(defaults, profile, 'mode', ['AGENT_LOOM_MODE'], 'agent'), ['agent', 'handoff', 'pro'], 'agent');

    const port = normalizePort(await ask(rl, 'Which local port should Agent Loom use?', defaultPort));
    ...
    printBox('Public URL', [
      ...
      'quick  = Agent Loom creates a Cloudflare quick tunnel for demos and local work.',
    ...
    const bash = optionValue(defaults, profile, 'bash', ['AGENT_LOOM_BASH_MODE'], '');
    const bashTranscript = bashTranscriptOption(defaults, profile);
    const codexSessions = codexSessionsOption(defaults, profile);
    const codexDir = optionValue(defaults, profile, 'codexDir', ['AGENT_LOOM_CODEX_DIR'], '');
    const write = optionalWriteOption(defaults, profile, mode);
    const toolMode = optionalChoice('tool-mode', optionValue(defaults, profile, 'toolMode', ['AGENT_LOOM_TOOL_MODE'], ''), ['minimal', 'standard', 'full']);
    const widgetDomain = optionValue(defaults, profile, 'widgetDomain', ['AGENT_LOOM_WIDGET_DOMAIN'], '');
    ...
    let profileToken = optionValue(defaults, profile, 'token', ['AGENT_LOOM_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], '');
    ...
    if (tunnelChoice === 'local') {
      ...
      const tunnelName = await ask(rl, 'Cloudflare tunnel name', optionValue(defaults, profile, 'tunnelName', ['AGENT_LOOM_TUNNEL_NAME', 'CLOUDFLARE_TUNNEL_NAME'], 'agent-loom'));
      profileTunnelName = tunnelName;
      args.push('--tunnel', 'cloudflare-named', '--hostname', hostname, '--tunnel-name', tunnelName);
      profileCloudflareConfig = optionValue(defaults, profile, 'cloudflareConfig', ['AGENT_LOOM_CLOUDFLARE_CONFIG', 'CLOUDFLARE_TUNNEL_CONFIG'], '');
      profileCloudflareTokenFile = optionValue(defaults, profile, 'cloudflareTokenFile', ['AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CLOUDFLARE_TUNNEL_TOKEN_FILE'], '');
      ...
      const ngrokConfig = optionValue(defaults, profile, 'ngrokConfig', ['NGROK_CONFIG', 'AGENT_LOOM_NGROK_CONFIG'], '');
      if (ngrokConfig) {
    ...
    if (profileTunnel !== 'none') {
      profileToken = await ask(rl, 'Agent Loom auth token for this workspace', stableToken(profileToken));
    ...
    const startAnswer = await ask(rl, 'Start Agent Loom now?', 'yes');
...
function printProfile(root, profile) {
  if (!profile.profilePath) {
    printBox('Agent Loom settings', [
      labelValue('Workspace', root),
      'No saved settings for this workspace.',
      'Run agent-loom settings set or agent-loom setup to save a tunnel preference.'
    ]);
    return;
  }
...
function printProfileList(profiles = listWorkspaceProfiles()) {
  if (!profiles.length) {
    printBox('Agent Loom saved setups', [
      'No saved workspace settings found.',
      'Run agent-loom setup or agent-loom settings set to create one.'
    ]);
    return;
  }
  printBox('Agent Loom saved setups', profiles.slice(0, 50).map((profile, index) => profileOneLine(profile, index + 1)));
}

function saveSettingsFromArgs(root, args, profile) {
  if (args.cloudflareToken !== undefined) {
    throw new Error('agent-loom settings set does not save raw --cloudflare-token. Save it to a local file and use --cloudflare-token-file <path>; start still accepts --cloudflare-token for a single launch.');
  }
  const tunnel = optionValue(args, profile, 'tunnel', ['AGENT_LOOM_TUNNEL'], profile.tunnel ?? 'cloudflare');
  ...
  const mode = optionValue(args, profile, 'mode', ['AGENT_LOOM_MODE'], profile.mode ?? 'agent');
  if (!['agent', 'handoff', 'pro'].includes(mode)) {
    throw new Error('--mode must be agent, handoff, or pro');
  }
  const toolMode = optionalChoice('tool-mode', optionValue(args, profile, 'toolMode', ['AGENT_LOOM_TOOL_MODE'], profile.toolMode ?? ''), ['minimal', 'standard', 'full']);
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['AGENT_LOOM_WIDGET_DOMAIN'], profile.widgetDomain ?? '');
  const port = normalizePort(optionValue(args, profile, 'port', ['AGENT_LOOM_PORT'], profile.port ?? '8787'));
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  const codexDir = optionValue(args, profile, 'codexDir', ['AGENT_LOOM_CODEX_DIR'], profile.codexDir ?? '');
  const { bashSession, requireBashSession } = bashSessionOptions(args, profile);
  const write = writeOption(args, profile, mode);
  const bash = optionalChoice('bash', optionValue(args, profile, 'bash', ['AGENT_LOOM_BASH_MODE'], profile.bash ?? ''), ['off', 'safe', 'full']);
  const tunnelName = tunnel === 'cloudflare-named' ? (args.tunnelName ?? profile.tunnelName ?? '') : '';
  const ngrokConfig = tunnel === 'ngrok'
    ? resolveConfigPath(root, optionValue(args, profile, 'ngrokConfig', ['NGROK_CONFIG', 'AGENT_LOOM_NGROK_CONFIG'], ''))
    : '';
  const cloudflareConfig = tunnel === 'cloudflare-named'
    ? resolveConfigPath(root, optionValue(args, profile, 'cloudflareConfig', ['AGENT_LOOM_CLOUDFLARE_CONFIG', 'CLOUDFLARE_TUNNEL_CONFIG'], ''))
    : '';
  const cloudflareTokenFile = tunnel === 'cloudflare-named'
    ? resolveConfigPath(root, optionValue(args, profile, 'cloudflareTokenFile', ['AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN_FILE', 'CLOUDFLARE_TUNNEL_TOKEN_FILE'], ''))
    : '';
  const token = tunnel === 'none'
    ? optionValue(args, profile, 'token', ['AGENT_LOOM_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], profile.token ?? '')
    : stableToken(optionValue(args, profile, 'token', ['AGENT_LOOM_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], profile.token ?? ''));
  const allowedRoots = configuredProjectRoots(root, args, profile);
  const savedPath = saveWorkspaceProfile(root, {
    port,
    mode,
...
async function runSettings(argv) {
  ...
  if (action === 'delete' || action === 'reset' || action === 'remove') {
    ...
    if (!args.yes && process.stdin.isTTY) {
      ...
      throw new Error('Use agent-loom settings delete --yes in non-interactive shells.');
    }
...
function writeControlPrompt() {
  process.stdout.write('agent-loom> ');
}
...
function waitForUnexpectedRuntimeExit(server, cleanup = cleanupChildren) {
  return new Promise((_, reject) => {
    const fail = (code, signal, error) => {
      ...
      reject(new Error(`Agent Loom HTTP runtime exited unexpectedly (${detail}).`));
    };
...
async function main() {
  ...
  if (profile.profilePath && !args.noProfile) {
    ...
    const summary = profileSummary(profile);
    if (summary) statusLine('ok', `${summary}. Future launches from this folder only need: agent-loom start`);
  }
  ...
  if (tunnel === 'ngrok' && !stableHostname) {
    throw new Error('--hostname is required with ngrok tunnel mode. Example: agent-loom ngrok --hostname your-domain.ngrok-free.dev');
  }
  if (tunnel === 'tailscale' && !stableHostname) {
    throw new Error('--hostname is required with Tailscale Funnel mode. Example: agent-loom tailscale --hostname your-device.your-tailnet.ts.net');
  }
  const mode = optionValue(args, profile, 'mode', ['AGENT_LOOM_MODE'], 'agent');
  if (!['agent', 'handoff', 'pro'].includes(mode)) {
    throw new Error('--mode must be agent, handoff, or pro');
  ...
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  const codexDir = resolveCodexDir(root, optionValue(args, profile, 'codexDir', ['AGENT_LOOM_CODEX_DIR'], ''));
  const { bashSession, requireBashSession } = bashSessionOptions(args, profile);
  const write = writeOption(args, profile, mode);
  const toolMode = optionValue(args, profile, 'toolMode', ['AGENT_LOOM_TOOL_MODE'], 'standard');
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['AGENT_LOOM_WIDGET_DOMAIN'], 'https://rebel0789.github.io');
  const toolCards = optionBool(args, profile, 'toolCards', ['AGENT_LOOM_TOOL_CARDS'], false);
  ...
  let token = args.noAuth
    ...
    : args.tokenFile
      ...
      : optionValue(args, profile, 'token', ['AGENT_LOOM_HTTP_TOKEN', 'CODEBASE_BRIDGE_HTTP_TOKEN'], '');
  if (!token && !args.noAuth) token = stableToken();

  const serverEnv = {
    ...process.env,
    AGENT_LOOM_ROOT: root,
    AGENT_LOOM_ALLOWED_ROOTS: allowRoots.join(path.delimiter),
    AGENT_LOOM_HOST: host,
    AGENT_LOOM_PORT: port,
    AGENT_LOOM_BASH_MODE: bash,
    AGENT_LOOM_BASH_TRANSCRIPT: bashTranscript,
    AGENT_LOOM_BASH_SESSION_ID: bashSession,
    AGENT_LOOM_REQUIRE_BASH_SESSION: requireBashSession ? '1' : '0',
    AGENT_LOOM_CODEX_SESSIONS: codexSessions,
    AGENT_LOOM_WRITE_MODE: write,
    AGENT_LOOM_TOOL_MODE: toolMode,
    AGENT_LOOM_WIDGET_DOMAIN: widgetDomain,
    AGENT_LOOM_TOOL_CARDS: toolCards ? '1' : '0',
    AGENT_LOOM_CONNECTION_TEST: connectionTest ? '1' : '0',
    AGENT_LOOM_MODE: mode,
    AGENT_LOOM_TUNNEL_MODE: tunnel === 'none' ? '0' : '1',
    AGENT_LOOM_ALLOW_NO_HTTP_TOKEN: args.noAuth ? '1' : '0'
  };
  if (codexDir) serverEnv.AGENT_LOOM_CODEX_DIR = codexDir;
  if (args.logRequests || process.env.AGENT_LOOM_LOG_REQUESTS === '1') serverEnv.AGENT_LOOM_LOG_REQUESTS = '1';
  if (args.allowHome) serverEnv.AGENT_LOOM_ALLOW_HOME = '1';
  if (token) serverEnv.AGENT_LOOM_HTTP_TOKEN = token;
  else delete serverEnv.AGENT_LOOM_HTTP_TOKEN;
  ...
  const verboseLogs = Boolean(args.logRequests || process.env.AGENT_LOOM_LOG_REQUESTS === '1');
  statusLine('wait', 'Starting local MCP server');
  const server = spawnLogged('agent-loom', process.execPath, [httpPath], { cwd: projectRoot, env: serverEnv, verbose: verboseLogs });
  let cloudflared;
  ...
  if (tunnel === 'ngrok') {
    ...
    try {
      ...
      const tail = typeof cloudflared.agentLoomLogTail === 'function' ? cloudflared.agentLoomLogTail() : '';
      const hint = [
        '',
        ...
        '  agent-loom ngrok --hostname your-domain.ngrok-free.dev --token keep-this-stable-token',
        '',
  ...
  if (tunnel === 'tailscale') {
    ...
    try {
      ...
      const hint = [
        ...
        '  agent-loom tailscale --hostname your-device.your-tailnet.ts.net --token keep-this-stable-token',
        '',
        'Funnel exposes this connector publicly. Keep the Agent Loom token enabled.'
      ].join('\n');
  ...
  const publicBase = publicBaseFromHostname(stableHostname);
  const tunnelName = optionValue(args, profile, 'tunnelName', ['CLOUDFLARE_TUNNEL_NAME', 'AGENT_LOOM_TUNNEL_NAME'], '');
  const cloudflareConfig = resolveConfigPath(root, optionValue(args, profile, 'cloudflareConfig', ['CLOUDFLARE_TUNNEL_CONFIG', 'AGENT_LOOM_CLOUDFLARE_CONFIG'], ''));
  const cloudflareTokenFile = resolveConfigPath(root, optionValue(args, profile, 'cloudflareTokenFile', ['CLOUDFLARE_TUNNEL_TOKEN_FILE', 'AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN_FILE'], ''));
  const cloudflareToken = optionValue(args, profile, 'cloudflareToken', ['CLOUDFLARE_TUNNEL_TOKEN', 'AGENT_LOOM_CLOUDFLARE_TUNNEL_TOKEN'], '');
  ...
  try {
    ...
    const hint = [
      ...
      '',
      '  --cloudflare-token-file ~/.agent-loom/cloudflare-tunnel-token',
      '',
...
main().catch((error) => {
  ...
  if (process.env.AGENT_LOOM_DEBUG === '1' && error instanceof Error && error.stack) {
